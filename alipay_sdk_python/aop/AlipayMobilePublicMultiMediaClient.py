...
from alipay_sdk_python.aop import AlipayMobilePublicMultiMediaExecute
...
class AlipayMobilePublicMultiMediaClient:
    ...
    def execute(self, header='', body='', httpCode=''):
        exe = AlipayMobilePublicMultiMediaExecute.AlipayMobilePublicMultiMediaExecute(header, body, httpCode)
        return exe
    ...
    def buildGetUrl(self, query=None):
        ...
        if not stripos(privateKey, "\n"):
            i = 0
            key_str = substr(privateKey, i * key_width, key_width)
            while key_str:
                key_str = substr(privateKey, i * key_width, key_width)
